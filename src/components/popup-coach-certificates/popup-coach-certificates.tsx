...
import {MAX_POPUP_CERTIFICATES_COUNT_PER_PAGE} from '../../const';

type PopupCoachCertificatesProps = {
  certificates: string[];
  setPopupOpened: (prevState: boolean) => void;
};

function PopupCoachCertificates({certificates, setPopupOpened}: PopupCoachCertificatesProps): JSX.Element {
  ...
  const handleRightArrowButtonClick = () => {
    const pagesCount = Math.ceil(certificates.length / MAX_POPUP_CERTIFICATES_COUNT_PER_PAGE);
          ...
          <div className="popup__wrapper">
            ...
            <div className="popup__content popup__content--certificates">
              ...
              <ul className="popup__slider-list">
                {
                  certificates.slice((certificatesPage - 1) * MAX_POPUP_CERTIFICATES_COUNT_PER_PAGE, ((certificatesPage - 1) * MAX_POPUP_CERTIFICATES_COUNT_PER_PAGE) + MAX_POPUP_CERTIFICATES_COUNT_PER_PAGE).map((certificateItem) => (
                    <li key={nanoid()} className="popup__slide popup__slide--current">
...
export default PopupCoachCertificates;
